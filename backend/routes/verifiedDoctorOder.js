...
router.post("", (req, res, next) => {
    ...
    VerifiedDocOder.save().then(createdDocOder => {
        res.status(201).json({
            message: 'Verified  Oder Added Successfully',
...
router.get("", (req, res, next) => {
    VerifiedDoctorOder.find().then(documents => {
        res.status(200).json({
            message: ' verify oder added sucessfully',
...
router.delete("/:id", (req, res, next) => {
    VerifiedDoctorOder.deleteOne({ _id: req.params.id }).then(result => {
        console.log(result);
        res.status(200).json({ message: ' verified order deleted!' });
    });
});
...
async function sendMail(user, callback) {
    ...
    <h3>You have picked up the oder from our Mangement TODAY</h3><br>
